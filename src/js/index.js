...
function displayProjects() {
  if (localStorage.getItem("projects")) {
    ...
    for (const [key, value] of Object.entries(projects)) {
      ...
      // to hold the projects todos once dragged and dropped
      let projectsTodos = document.createElement("div");
      projectsTodos.classList.add("projects-todos");
      newProject.appendChild(projectsTodos);
    }
  }
}

function displayProjectsTodos() {
  if (localStorage.getItem("projects")) {
    let projects = JSON.parse(localStorage.getItem("projects"));
    for (const [key, value] of Object.entries(projects)) {
      for (let todo of value.todos) {
        console.log(todo);
      }
    }
  }
